...
**Your own intelligence terminal. 33 sources. One command. Zero cloud.**
...
[![Sources](https://img.shields.io/badge/OSINT%20sources-33-cyan)](#data-sources-33)
...
Crucix pulls satellite fire detection, flight tracking, radiation monitoring, satellite constellation tracking, economic indicators, live market prices, conflict data, sanctions lists, and social sentiment from 33 open-source intelligence feeds — in parallel, every 15 minutes — and renders everything on a single self-contained Jarvis-style dashboard.
...
The dashboard opens automatically at `http://localhost:3117` and immediately begins its first intelligence sweep. This initial sweep queries all 33 sources in parallel and typically takes 30–60 seconds — the dashboard will appear empty until the sweep completes and pushes the first data update. After that, it auto-refreshes every 15 minutes via SSE (Server-Sent Events). No manual page refresh needed.
...
### Situational Map
A second view at `/map` — a MapLibre GL app over a free CARTO dark-matter basemap (no API key), built for a single-glance world picture rather than the full Jarvis HUD:
- Toggleable layers: earthquakes, volcanoes, wildfires, severe storms, fire detections, severe weather alerts, RadNet stations, Ukraine frontline (fill + line), conflict events, GPS/GNSS jamming zones, maritime chokepoints, air traffic hotspots, live ISS position, tracked space stations, GDELT news events, prediction markets
- Click any marker or frontline shape for a detail popup
- Live via the same SSE feed as the main dashboard — no separate polling

### Auto-Refresh
The server runs a sweep cycle every 15 minutes (configurable). Each cycle:
1. Queries all 33 sources in parallel (~30s)
...
│
├── apis/
│   ├── briefing.mjs           # Master orchestrator — runs all 33 sources in parallel
...
│   └── sources/               # 33 self-contained source modules
...
- **Parallel execution** — `Promise.allSettled()` fires all 33 sources simultaneously
...
### Tier 7: Live Map Layers (5)

Feeds the [Situational Map](#situational-map) at `/map` — MapLibre GL over a CARTO basemap.

| Source | What It Tracks | Auth |
|--------|---------------|------|
| **USGS** | Real-time earthquakes (M2.5+, past week) | None |
| **NASA EONET** | Open wildfires, volcanic activity, severe storms | None |
| **DeepStateMap.live** | Community-maintained Ukraine frontline geometry | None |
| **GPSJam.org** | GPS/GNSS jamming zones (derived from ADS-B Exchange nav-integrity data) | None |
| **Polymarket** | Geopolitics/macro prediction markets, geo-tagged by title | None |
...
| `GET /map` | MapLibre situational map (see [Situational Map](#situational-map)) |
...
This is normal — the first sweep takes 30–60 seconds to query all 33 sources. The dashboard will populate automatically once the sweep completes. Check the terminal for sweep progress logs.
